/* 
  ...
  Add the random in page function by shixing
...
static double run_randlatinpage(long long size, int ITS)
{
	...
	for (int i=2;i<argc;i++)
	{
		...
		else if (!strcmp ("-t", argv[i]))
		{
			...
			if (!strcmp (argv[i], "randlat"))
				...
				opts.test = TEST_DTLB;
			else if(!strcmp (argv[i], "randinpagedlat"))
				opts.test = TEST_RANDINPAGELAT;
			else
			{
...
int main(int argc, char ** argv)
{
	...
	switch (opts.test)
	{
		...
		case TEST_RANDINPAGELAT: printf ("Random in page access latency test\n"); break;
	...
	else if (opts.random_in_page) // random in page mode increase n*page size of the array
	{
		for (long long size = 512, step=512; size  <= opts.sizelimit/sizeof(void*); size = size + step )
		{
			double cpi = runtest(opts, size, its);
